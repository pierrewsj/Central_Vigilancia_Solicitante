...
async function processSolicQueue(){
  ...
  if(done.length){ showModal('Sincronização concluída',`${done.length} solicitação(ões) pendente(s) foram enviadas para a planilha.<br>Número(s): <strong>${done.filter(Boolean).join(', ')}</strong>`,'✅',true); }
}
window.addEventListener('online',()=>{setOfflineState();processSolicQueue();});
...
async function salvarChamado(origem){
  ...
  try{
    ...
    showModal('Chamado registrado',`Número: <strong>${c.id}</strong><br>O chamado foi salvo com sucesso e já pode ser consultado no aplicativo.`,'✅',true);
    if(origem==='bot'){ $('consultaId').value=c.id; }
  }catch(e){
