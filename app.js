...
function renderStatusAlert(id,status,updatedAt,isNew=false){
  ...
  box.innerHTML = `<div><strong>🔔${destaque}</strong><br>Chamado <strong>${id}</strong>${status?` está como <strong>${status}</strong>`:''}.${hora}</div><button class="btn secondary mini-btn" onclick="go('view-status');consultarStatus()">Ver status</button>`;
