...
function go(id,btn){ document.querySelectorAll('.view').forEach(v=>v.classList.remove('active')); $(id).classList.add('active'); document.querySelectorAll('.navbtn').forEach(b=>b.classList.remove('active')); if(btn)btn.classList.add('active'); window.scrollTo({top:0,behavior:'smooth'}); if(id==='view-form') setTimeout(focusFirstVisible,180); if(id==='view-bot') setTimeout(focusBotInput,220); }
...
function focusBotInput(){ const input=$('botInput'); const composer=$('chatComposer'); if(!input) return; setTimeout(()=>{ try{ input.focus({preventScroll:true}); }catch(e){ input.focus(); } if(composer) composer.scrollIntoView({behavior:'smooth', block:'end'}); }, 80); }
