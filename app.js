...
function saveSolicQueue(q){ localStorage.setItem(SOLIC_QUEUE_KEY, JSON.stringify(q)); }
...
function salvarUltimoChamado(id,status='Recebido',updatedAt=''){
  ...
  if(box){ box.innerHTML=`Último chamado gerado: <strong>${id}</strong>`; box.classList.remove('hidden'); }
...
function carregarUltimoChamado(){
  ...
  if(id){
    ...
    const box=$('ultimoChamadoBox'); if(box){ box.innerHTML=`Último chamado gerado: <strong>${id}</strong>`; box.classList.remove('hidden'); }
...
function renderStatusAlert(id,status,updatedAt,isNew=false){
  ...
  box.innerHTML = `<div><strong>🔔${destaque}</strong><br>Chamado <strong>${id}</strong>${status?` está como <strong>${status}</strong>`:''}.${hora}</div><button class="btn secondary mini-btn" onclick="go('view-status',document.querySelectorAll('.navbtn')[3]);consultarStatus()">Ver status</button>`;
...
async function processSolicQueue(){
  ...
  if(done.length){ const gerados=done.filter(Boolean); if(gerados.length) salvarUltimoChamado(gerados[gerados.length-1],'Recebido'); showModal('Sincronização concluída',`${done.length} solicitação(ões) pendente(s) foram enviadas para a planilha.<br>Número(s): <strong>${gerados.join(', ')}</strong>`,'✅',true); }
}
...
function init(){
  ...
  renderSteps(); onTipoChange(); toggleRamal(); toggleEmpresa(); toggleColuna(); toggleSala(); carregarUltimoChamado(); startStatusWatcher();
...
async function salvarChamado(origem){
  ...
  if(!navigator.onLine){
    ...
    showModal('Sem internet',`A solicitação foi salva neste aparelho como <strong>${localId}</strong> e será enviada automaticamente para a planilha quando o sinal voltar.`,'⚠️',true);
  ...
  try{
    ...
    showModal('Chamado registrado',`Número: <strong>${c.id}</strong><br>O número foi preenchido automaticamente na consulta de status.`,'✅',true);
  }catch(e){
    const localId=queueSolicitacao(c);
    showModal('Salvo para sincronizar',`Não foi possível enviar agora. A solicitação ficou salva neste aparelho como <strong>${localId}</strong> e será sincronizada automaticamente quando a conexão voltar.<br><br>${String(e.message||e)}`,'⚠️',true);
  }
